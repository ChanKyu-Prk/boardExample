...
public class BoardDaoImpl implements BoardDao {
   ...
   public BoardDto findBySeq(int seq) {
      ...
      BoardDto dto = (BoardDto) jdbcTemplate.query(sql,
            new BoardPreparedStatementSetterForPrimaryKey(seq),
            new UserResultSetExtractor());
      return dto;
   }

   // 쿼리문 파라미터에 값을 매칭시키는 내부 클래스
   private class BoardPreparedStatementSetterForPrimaryKey
   ...
   private class UserResultSetExtractor implements ResultSetExtractor {
      ...
      public Object extractData(ResultSet rs) throws SQLException, DataAccessException {
         // 직접 하나 하나 처리해야함. RowMapper 는 스프링이 반복해서 처리해줌.
         if (rs.next()) {
            BoardDto dto = new BoardDto();
            dto.setSeq(rs.getInt("seq"));
            dto.setTitle(rs.getString("title"));
   ...
   public void insert(BoardDto dto) {
      String sql = "insert into tblspringboard (title, content, writer, "
            + " regdate, hitcount, password) "
            + " values (?, ?, ?, now(), 0, '1234')";
      jdbcTemplate.update(sql,
            new BoardPreparedStatementSetterForInsert(dto.getTitle(),
   ...
   public void update(BoardDto dto) {
      String sql = "update tblspringboard set title=?, content=? "
    		  	+ " where seq=?";
      Object[] val = new Object[] { dto.getTitle(), dto.getContent(),
            dto.getSeq() };
